...
pdApp.controller(
	...
	function (
		...
		var loadOrder = function () {
			...
			$scope.productsSorted.push({
				type: "Persiana o Filtrasol",
				products: [],
			});
			...
			$scope.limitDays = 20;
			$scope.maxDate;
			paldiService.orders.get(id).then(
				function (order) {
					$scope.order = order;
					...
					if (order.type === "Mixta") {
						$scope.isMaster = true;
						order.mixedLabel !== null
							? ($scope.mixedLabel = order.mixedLabel)
							: ($scope.mixedLabel = "Mixta");
						paldiService.orders
					...
					$scope.cycleStartDate = order.cycleStartDate
						? moment(order.cycleStartDate)
						: null;
					$scope.cycleFinishDate = order.cycleFinishDate
						? moment(order.cycleFinishDate)
						: null;
					$scope.commitmentDate = order.commitmentDate
						? moment(order.commitmentDate)
						: null;
					$scope.productionStartDate = order.productionDate
						? moment(order.productionDate)
						: null;
					$scope.productionFinishDate = order.productionFinishDate
						? moment(order.productionFinishDate)
						: null;
					$scope.productionLimitDate = order.productionLimitDate
						? moment(order.productionLimitDate)
						: null;
					$scope.transitStartDate = order.transitDate
						? moment(order.transitDate)
						: null;
					$scope.transitFinishDate = order.transitFinishDate
						? moment(order.transitFinishDate)
						: null;
					$scope.transitLimitDate = order.transitLimitDate
						? moment(order.transitLimitDate)
						: null;
					$scope.currentDate = moment();
					$scope.cycleDays = getCycleDays(
						$scope.currentDate,
						$scope.cycleStartDate,
						$scope.cycleFinishDate
					);
					$scope.dpfcStatus = "";
					$scope.dpfcTotalStatus = "";
					$scope.dpfcTotalDays = 0;
					$scope.dpfcDays = getDPFCDays(
						$scope.cycleStartDate,
						$scope.commitmentDate,
						$scope.cycleFinishDate
					);
					$scope.productionStatus = "";
					$scope.productionTotalStatus = "";
					$scope.productionTotalDays = "-";
					$scope.productionDays = getProductionDays(
						$scope.productionStartDate,
						$scope.productionLimitDate,
						$scope.productionFinishDate
					);

					$scope.transitDays = getTransitDays(
						$scope.currentDate,
						$scope.transitStartDate,
						$scope.transitFinishDate,
						$scope.transitLimitDate
					);
		...
		var orderProductsByType = function (product) {
			var pos;
			if (
				$scope.mixedLabel != undefined &&
				$scope.mixedLabel.indexOf("Persianas") != -1 &&
				$scope.mixedLabel.indexOf("Filtrasol") != -1
			) {
				if (
