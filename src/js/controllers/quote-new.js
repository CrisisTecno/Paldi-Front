...
pdApp.controller(
  ...
  function (
    ...
    $scope.addProduct = function (product, form, model) {
      // addProduct(productName, undefined, undefined) is called when adding a new product
      // from the quote-new view buttons
      if (!form) {
        addNewProduct($scope, product);
      }

      // addProduct(productName, productDetails, $scope.{product}) is called when
      // a product is added from the views/console/quote/{product}.html
      if (form) {
    ...
    $scope.removeProduct = function (product, indexList, indexProduct) {
      ...
      if ($scope.isMultiple) {
        ...
        $scope.productsSorted.forEach(function (product) {
          if (product.products.length > 0) countProducts++;
        });
      ...
      // colorPriceService.updateTotals($scope.quote.type, $scope.quote);
      // colorPriceService.updateTotals($scope.quote.type, $scope.productsSorted[indexList].products[indexProduct]);
    };
