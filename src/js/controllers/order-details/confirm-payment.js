...
const getAdvance = (model) => model.currency === 'DOLLARS' ? parseFloat(model.advance * model.exchangeRate).toFixed(2) : model.advance
const getAdvanceDollars = (model) => model.currency == 'DOLLARS' ? model.advance : null
const getExchangeRate = (model) => model.currency == "DOLLARS" ? model.exchangeRate : null
const getBankPaymentType = (model) => model.type == "BANK" ? model.bankType : null

const getProps = (model) => ({
  title: `¿Seguro que deseas enviar la orden con $${model.advance} ${model.currency == "PESOS" ? "M.N." : "Dlls"}?`,
  type: "warning",
...
const updateOrder = async function (context, $scope, updatedOrder) {
  try {
    const callback = async function () {
      const order = await context.paldiService.orders.updateStatus(updatedOrder, "LINE")
...
const getPaymentSwalHandler = (context, $scope) => function (model, confirm) {
  context = $scope
  console.log($scope)
...
export const getConfirmPayment = (context, $scope) => {
  ...
  return (model) => {
    swal(getProps(model), function (confirm) { paymentSwalHandler(model, confirm) });
  }
}