...
(function (window, document, $, angular) {
	...
	function dataTable(
		...
		function compileDirective(tElm) {
			...
			return function postLink($scope, $elem, iAttrs, ctrl) {
				...
				var watchFunction = iAttrs.dtDisableDeepWatchers
					? "$watchCollection"
					: "$watch";
				angular.forEach(
					["dtColumns", "dtColumnDefs", "dtOptions"],
					function (tableDefField) {
						$scope[watchFunction].call(
							$scope,
							tableDefField,
							handleChanges,
							true
		...
		function ControllerDirective($scope) {
			...
			function buildOptionsPromise() {
				...
				$q.all([
					...
					.then(function (results) {
						...
						return DTPropertyUtil.resolveObjectPromises(options, [
							"data",
							"aaData",
							"fnPromise",
						]);
			...
			function render($elem, optionsPromise, staticHTML) {
				optionsPromise.then(function (options) {
					...
					var isNgDisplay =
						$scope.datatable && $scope.datatable === "ng";
					// Render dataTable
					if (_dtInstance && _dtInstance._renderer) {
						_dtInstance._renderer
							.withOptions(options)
							.render($elem, $scope, staticHTML)
	...
	angular
		.module("datatables.instances", ["datatables.util"])
	...
	angular
		.module("datatables", ["datatables.directive", "datatables.factory"])
		...
		if ($.fn.DataTable.Api) {
			...
			$.fn.DataTable.Api.register("ngDestroy()", function (remove) {
				...
				return this.iterator("table", function (settings) {
					...
					$.fn.DataTable.ext.internal._fnCallbackFire(
						settings,
						"aoDestroyCallback",
						"destroy",
						[settings]
	...
	function dtRendererService(DTLoadingTemplate) {
		...
		function showLoading($elem, $scope) {
			var $loading = angular.element(
				DTLoadingTemplate.compileHtml($scope)
			);
			$elem.after($loading);
			$elem.hide();
			$loading.show();
	...
	function dtNGRenderer(
		...
		function create(options) {
			...
			function render($elem, $scope, staticHTML) {
				...
				var defer = $q.defer();
				var _$tableElem = _staticHTML.match(
					/<tbody([\s\S]*)<\/tbody>/i
				);
				var _expression = _$tableElem[1];
				// Find the resources from the comment <!-- ngRepeat: item in items --> displayed by angular in the DOM
				// This regexp is inspired by the one used in the "ngRepeat" directive
				var _match = _expression.match(
					/^\s*.+?\s+in\s+([a-zA-Z0-9\.-_$]*)\s*/m
				);
					...
					function () {
						...
						$timeout(
							function () {
								...
								var result =
									DTRendererService.hideLoadingAndRenderDataTable(
										_$elem,
										renderer.options
									);
								_oTable = result.DataTable;
								DTInstanceFactory.copyDTProperties(
									result,
									dtInstance
								);
			...
			function changeData() {
				$log.warn(
					"The Angular Renderer does not support changing the data. You need to change your model directly."
				);
			}
	...
	function dtPromiseRenderer(
		...
		function create(options) {
			...
			function render($elem, $scope) {
				...
				_$scope = $scope;
				_resolve(
					renderer.options.fnPromise,
					DTRendererService.renderDataTable
			...
			function reloadData(callback, resetPaging) {
				var previousPage =
					_oTable && _oTable.page() ? _oTable.page() : 0;
				if (angular.isFunction(renderer.options.fnPromise)) {
					...
					$log.warn(
						"In order to use the reloadData functionality with a Promise renderer, you need to provide a function that returns a promise."
					);
				}
			}
			...
			function _startLoading(fnPromise, callback) {
				...
				_loadedPromise.then(function (result) {
					...
					if (renderer.options.sAjaxDataProp) {
						var properties =
							renderer.options.sAjaxDataProp.split(".");
					...
					_loadedPromise = null;
					defer.resolve(
						_doRender(renderer.options, _$elem, data, callback)
					);
				});
				return defer.promise;
			}
		...
		function create(options) {
			...
			function render($elem, $scope) {
				...
				if (angular.isUndefined(renderer.options.sAjaxDataProp)) {
					renderer.options.sAjaxDataProp =
						DT_DEFAULT_OPTIONS.sAjaxDataProp;
				}
	...
	function dtRendererFactory(
		...
		function fromOptions(options, isNgDisplay) {
			...
			if (angular.isDefined(options)) {
				if (
					angular.isDefined(options.fnPromise) &&
			...
			if (angular.isObject(target)) {
				for (var prop in target) {
					if (target.hasOwnProperty(prop)) {
						result[prop] = overrideProperties(
							result[prop],
							target[prop]
						);
					}
		...
		function resolveObjectPromises(obj, excludedPropertiesName) {
			...
			if (!angular.isObject(obj) || angular.isArray(obj)) {
				...
				for (var prop in resolvedObj) {
					if (
						resolvedObj.hasOwnProperty(prop) &&
						$.inArray(prop, excludedProp) === -1
					) {
						if (angular.isArray(resolvedObj[prop])) {
							promises.push(
								resolveArrayPromises(resolvedObj[prop])
							);
						} else {
